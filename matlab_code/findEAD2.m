function EAD = findEAD2(t,V,t_cutoff,flag)
EAD = 0;
...
nonEADs = find(time_between_depols > 20);
if ~isempty(nonEADs)
...
if any(time_between_depols > t_cutoff)
    %if (time_between_depols(1) > time_between_depols(end))
        ...
        %         hold on
        %         plot(time,Voltage)
        %         plot(time(above_threshold(time_between_depols)),Voltage(above_threshold(time_between_depols)),'go')
        %         inds = find(time_between_depols > t_cutoff);
        %         temp = find(above_threshold(time_between_depols) == 1);
